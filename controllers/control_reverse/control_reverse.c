...
int main(int argc, char **argv) {
  ...
  while (wb_robot_step(TIME_STEP) != -1) {
    ...
    double th2 = theta[1];//wb_position_sensor_get_value(theta2);
    double phi = beta - (theta[0]+1.5708);
    double thetadeg = 90+(theta[0]*180/3.1416);
    double betadeg = beta*180/3.1416;
    double phideg = phi*180/3.1416;
    double th2deg = (th2*180/3.1416);
     ...
     */
     ...
     if (tiempo < 42){
       if (pos[0] < sp){
        ...
        // thsteer = (((((ms+mp)*rs)+(mb*(rs-rb)))*((Vd*rs)*(Vd*rs)))+(mb*g*rb*rs))/(mb*g*rb*rc);
         double thsteer = (((m1+m2)*r1+m2*(r1-r2))*(thetap[0]*thetap[0]*Vd*r1*r1)+m2*g*r2*r1)/(m2*g*r2*rc);
        ...
        // double thsteer = atan(rs/rc);
         ...
         wb_motor_set_torque(longitudinal, newVel);
         rc = (0-spz)/2;
         double thsteer = ((r1*thetap[0]*thetap[0])*(ic-m2*r1*r2+r1*r1*(m1+m2)))/(m2*g*r2*rc);
         //double thsteer = (((m1+m2)*r1+m2*(r1-r2))*(Vd*Vd*r1*r1)+m2*g*r2*r1)/(m2*g*r2*rc);
         thsteer = (1 - 0.01)*(thsteer - 0.032*thetap[0])+0.01*th2;
         wb_motor_set_position(steer, thsteer);
         wb_motor_set_velocity(steer, 1.5);
     ...
     printf("W = %frad/s\n", thetap[1]);
     printf("Wsph = %frad/s\n", phip[0]);
     printf("Debug = %f\n", thsteer);//the*180/3.1416); //thsteer*180/3.1416);
